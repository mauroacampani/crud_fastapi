...
from .schemas import Book, BookCreateModel, BookUpdateModel, BookReviewsModel
...
@book_router.get('/{book_uid}', response_model=BookReviewsModel, dependencies=[role_checker])
async def get_books(book_uid: str, session: AsyncSession = Depends(get_session), token_details = Depends(access_token_bearer)) -> dict:
...
async def update_books(book_uid: str, book_update_data: BookUpdateModel, session: AsyncSession = Depends(get_session), token_details = Depends(access_token_bearer)) -> dict:
...
async def delete_books(book_uid: str, session: AsyncSession = Depends(get_session), token_details = Depends(access_token_bearer)) -> dict:
